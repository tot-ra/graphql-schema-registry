...
type PotentialScalarTypes = ObjectTypeDefinitionNode
	| InputObjectTypeDefinitionNode
	| DirectiveDefinitionNode
	| EnumTypeDefinitionNode;

export class ScalarStrategy implements TypeDefStrategy<ScalarTypeExtensionNode> {
	...
	private getExistingScalars(data: ITypeDefData): ScalarTypeExtensionNode[] {
		...
		return types.map(type => {
			return [
				..."fields" in type ? type.fields.map(i => this.getInternalScalars(i.type)) : [],
				..."arguments" in type ? type.arguments.map(a => this.getInternalScalars(a.type)) : []
			]
		})
		.flat(1);
	}
	...
	private getInternalScalars(field: any): ScalarTypeExtensionNode {
		...
		return {
			kind: "ScalarTypeExtension", name: field.name
		}
