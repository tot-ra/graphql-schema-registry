...
export class InputStrategy implements TypeDefStrategy<InputObjectTypeExtensionNode> {
	...
	private async getInputValues(data: ITypeDefData, entities: InputObjectTypeExtensionNode[]) {
		const parentFields = entities.map(e => {
			return e.fields.map(f => createField(f, e.name.value, data)).flat(1)
		}).flat(1);
