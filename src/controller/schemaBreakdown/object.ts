...
export class ObjectStrategy
	implements TypeDefStrategy<ObjectTypeExtensionNode>
{
	...
	private implementationRepository =
		ImplementationTransactionRepository.getInstance();
	private operationsRepository =
		OperationTransactionalRepository.getInstance();
	private operationsParamsRepository =
	...
	async insertEntities(
		data: ITypeDefData,
		entities: ObjectTypeExtensionNode[]
	): Promise<void> {
		if (entities.length === 0) {
			return;
		}
		const notOperations = entities.filter(
			(obj) => !this.INVALID_OBJECTS.includes(obj.name.value)
		);
		const operations = entities.filter((obj) =>
			this.INVALID_OBJECTS.includes(obj.name.value)
		);
		...
		const dbTypes: Type[] = await this.typeRepository.getTypesByNames(
			data.trx,
			notOperations.map((e) => e.name.value)
		);
	...
	private getObjectFields(
		...
		return entities
			.map((o) => {
				const baseFields = o.fields.map((f) =>
					createField(f, o.name.value, data)
				);
				const argumentFields = o.fields
					.map((f) => {
						if (f.arguments?.length > 0) {
							return f.arguments.map((a) =>
								createField(a, o.name.value, data)
							);
						}
					})
					.flat(1)
	...
	private async getArgumentFields(
		data: ITypeDefData,
		entities: ObjectTypeExtensionNode[],
		dbFields: FieldPayload[]
	): Promise<Argument[]> {
		const fields = await this.fieldRepository.getFieldsByNames(
			data.trx,
		...
		return entities
			.map((o) => {
				const baseFields = o.fields.filter(
					(f) => f.arguments?.length > 0
				);
				return baseFields
					.map((f) => {
						return f.arguments
	...
	private getOperations(
		...
		return [
			...createOperations(
				queries.map((o) => o.fields).flat(1),
				OperationType.QUERY,
				data.service_id
			),
			...createOperations(
				mutations.map((o) => o.fields).flat(1),
			...
			.map((o) => {
				return o.fields
					.map((f) => {
						...
						const output = createOperationParam(
							f,
							parentName,
							true,
							data
						);
						const inputs = f.arguments.map((a) =>
							createOperationParam(a, parentName, false, data)
						);
