...
export class OperationParamsTransactionalRepository implements OperationParamsService {
	...
	async insertIgnoreOperationParams(data: OperationParam[]) {
		return this.trx
			.raw(`INSERT IGNORE INTO ${this.tableName} (name, is_nullable, is_array, is_array_nullable, is_output, operation_id, type_id) VALUES ${OperationParamsTransactionalRepository.insertBulkPayload(data)}`)
	}

	private static insertBulkPayload(data: OperationParam[]): string {
		const insertData = data.map(i => {
			return `('${i.name}', ${i.is_nullable ? 1 : 0}, ${i.is_array ? 1 : 0}, ${i.is_array_nullable ? 1 : 0}, ${i.is_output ? 1 : 0}, ${i.operation_id}, ${i.type_id})`
		})
