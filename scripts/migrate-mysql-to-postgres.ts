...
const basePg = {
	...
	password: process.env.MIGRATION_PG_PASSWORD || process.env.DB_SECRET || 'postgres',
	database: process.env.MIGRATION_PG_DB || process.env.DB_NAME || 'schema_registry',
};

async function main() {
	const skipMigrations =
		(process.env.MIGRATION_SKIP_PG_MIGRATIONS || 'false').toLowerCase() === 'true';
